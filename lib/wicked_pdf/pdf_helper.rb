module WickedPdf
  module PdfHelper
    ...
    def render_with_wicked_pdf(options = {}, *args, &block)
      if !options[:text] && (formats.first == :pdf || (options.is_a?(Hash) && options.has_key?(:pdf)))
        log_pdf_creation
        options[:basic_auth] = set_basic_auth(options)
        make_and_send_pdf(options[:filename] || params[:action], (WickedPdf.config || {}).merge(options))
      else
    ...
    def make_pdf(options = {})
      html_string = render_to_string_without_wicked_pdf(:template => options[:template], :layout => options[:layout])
      options = prerender_header_and_footer(options)
      w = Renderer.new(options[:wkhtmltopdf])
    ...
    def make_and_send_pdf(pdf_name, options={})
      ...
      if options[:show_as_html]
        render_without_wicked_pdf :template => options[:template], :layout => options[:layout], :content_type => "text/html"
      else
        pdf_name += '.pdf' unless pdf_name =~ /\.pdf\Z/
